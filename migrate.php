...
try {
    ...
    function record_exists(PDO $pdo, $table, $id) {
    ...
    if (file_exists($dataDir . 'groups.json')) {
        ...
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
            $pk = $item['id'] ?? $item['uuid'] ?? $item['union_id'] ?? null;
            if (empty($pk)) {
                log_migration_error($logSkipFile, 'unions', 'missing id', $item);
                $skipped++;
                continue;
            }
            if (safe_execute($stmt, [$pk, $item['name'] ?? null, $item['branch_id'] ?? null, $item['description'] ?? null, $item['status'] ?? 'active'], 'unions', $logErrorFile)) {
                $inserted++;
            }
        }
    ...
    if (file_exists($dataDir . 'assignments.json')) {
        ...
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
            $union = $item['union'] ?? $item['union_uuid'] ?? null;
            $branch = $item['branch'] ?? $item['branch_id'] ?? null;
            // require at least co and union to identify assignment
            if (empty($item['co']) || empty($union)) {
                log_migration_error($logSkipFile, 'assignments', 'missing co or union', $item);
                $skipped++;
                continue;
            }
            safe_execute($stmt, [$item['co'] ?? null, $union, $branch, $item['assigned_at'] ?? null], 'assignments', $logErrorFile);
            $inserted++;
        }
    ...
    if (file_exists($dataDir . 'users.json')) {
        $data = json_decode(file_get_contents($dataDir . 'users.json'), true) ?: [];
        $stmt = $pdo->prepare("INSERT INTO `users` (id, name, username, password, role, profile_pic, zone_id, area_id, branch_id, email, phone, last_login, is_weekly, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name), role = VALUES(role), profile_pic = VALUES(profile_pic), zone_id = VALUES(zone_id), area_id = VALUES(area_id), branch_id = VALUES(branch_id), email = VALUES(email), phone = VALUES(phone), last_login = VALUES(last_login), is_weekly = VALUES(is_weekly)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
            $pk = $item['id'] ?? null;
            $username = $item['username'] ?? null;
            if (empty($pk) || empty($username)) {
                log_migration_error($logSkipFile, 'users', 'missing id or username', $item);
                $skipped++;
                continue;
            }
            safe_execute($stmt, [$pk, $item['name'] ?? null, $username, $item['password'] ?? null, $item['role'] ?? null, $item['profile_pic'] ?? null, $item['zone_id'] ?? null, $item['area_id'] ?? null, $item['branch_id'] ?? null, $item['email'] ?? null, $item['phone'] ?? null, $item['last_login'] ?? null, $item['is_weekly'] ?? 0, $item['created_at'] ?? null], 'users', $logErrorFile);
            $inserted++;
        }
        $results['users'] = $inserted . ' (skipped:' . $skipped . ')';
    }
    ...
    if (file_exists($dataDir . 'clients.json')) {
        ...
        $stmt = $pdo->prepare("INSERT INTO clients (id, name, phone, email, address, `union`, branch_id, officer_username, client_type, plan_id, status, photo, id_type, id_number, guarantor_name, guarantor_phone, guarantor_address, date_registered) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
            $pk = $item['id'] ?? null;
            if (empty($pk)) {
    ...
    if (file_exists($dataDir . 'disbursements.json')) {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM disbursements WHERE id = ?");
        $stmt = $pdo->prepare("INSERT INTO disbursements (id, client_id, client_name, officer, branch_id, principal, interest_rate, total_payable, remaining_balance, num_installments, loan_term_type, client_type, date, due_date, payoff_date, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
    ...
    if (file_exists($dataDir . 'collections.json')) {
        ...
        $checkStmt = $pdo->prepare("SELECT transaction_id FROM loan_collections WHERE transaction_id = ?");
        $stmt = $pdo->prepare("INSERT INTO loan_collections (transaction_id, client_id, disbursement_id, amount_collected, date, officer, type, disbursement_date, remaining_balance, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
    ...
    if (file_exists($dataDir . 'Saving_Balance.json')) {
        ...
        $updateStmt = $pdo->prepare("UPDATE saving_balances SET balance = ? WHERE client_id = ?");
        $insertStmt = $pdo->prepare("INSERT INTO saving_balances (client_id, balance) VALUES (?, ?)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $clientId => $item) {
            ...
            try {
                ...
                log_migration_error($logErrorFile, 'saving_balances', $e->getMessage(), ['client_id' => $clientId, 'balance' => $balance]);
                $skipped++;
            }
        }
    ...
    if (file_exists($dataDir . 'savings.json')) {
        ...
        $checkStmt = $pdo->prepare("SELECT transaction_id FROM saving_collections WHERE transaction_id = ?");
        $stmt = $pdo->prepare("INSERT INTO saving_collections (transaction_id, client_id, savings_id, amount, type, date, officer, balance_after, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
            $pk = $item['transaction_id'] ?? $item['id'] ?? null;
            if (empty($pk)) {
        ...
        try {
            ...
            while ($row = $balancesStmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                try {
                    ...
                    log_migration_error($logErrorFile, 'saving_balances_compute', $e->getMessage(), $row);
                }
            }
            $results['computed_saving_balances'] = $updated;
        } catch (Exception $e) {
            log_migration_error($logErrorFile, 'saving_balances_compute', $e->getMessage(), null);
        }
    }
    ...
    if (file_exists($dataDir . 'notifications.json')) {
        ...
        $stmt = $pdo->prepare("INSERT INTO notifications (users, title, message, type, is_read) VALUES (?, ?, ?, ?, ?)");
        foreach ($data as $item) {
