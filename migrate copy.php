...
try {
    ...
    if (file_exists($dataDir . 'users.json')) {
        ...
        $stmt = $pdo->prepare("INSERT INTO users (id, name, username, password, role, profile_pic, zone_id, area_id, branch_id, email, phone, last_login, is_weekly, weekly_interest_rate, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name), password = VALUES(password), role = VALUES(role), profile_pic = VALUES(profile_pic), zone_id = VALUES(zone_id), area_id = VALUES(area_id), branch_id = VALUES(branch_id), email = VALUES(email), phone = VALUES(phone), last_login = VALUES(last_login), is_weekly = VALUES(is_weekly), weekly_interest_rate = VALUES(weekly_interest_rate)");
        $inserted = 0; $skipped = 0;
        foreach ($data as $item) {
            $pk = $item['id'] ?? null;
            if (empty($pk)) {
                log_migration_error($logSkipFile, 'users', 'missing id', $item);
                $skipped++;
                continue;
            }
            safe_execute($stmt, [$pk, $item['name'] ?? null, $item['username'] ?? null, $item['password'] ?? null, $item['role'] ?? null, $item['profile_pic'] ?? null, $item['zone_id'] ?? null, $item['area_id'] ?? null, $item['branch_id'] ?? null, $item['email'] ?? null, $item['phone'] ?? null, $item['last_login'] ?? null, $item['is_weekly'] ?? 0, $item['weekly_interest_rate'] ?? null, $item['created_at'] ?? null], 'users', $logErrorFile);
            $inserted++;
        }
