...
class MobileDashboardController extends Controller
{
    public function stats(Request $request)
    {
        ...
        $clients = DB::table('clients')->where('officer_username', $username)->where('status', 'active')->count();
        ...
        $collectedToday = (float) DB::table('loan_collections')->where('officer', $username)->whereDate('date', today())->sum('amount_collected');
        $collectedMonth = (float) DB::table('loan_collections')->where('officer', $username)->whereRaw("DATE_FORMAT(date,'%Y-%m') = ?", [$month])->sum('amount_collected');
        $savingsToday = (float) DB::table('saving_collections')->where('officer', $username)->where('type', 'deposit')->whereDate('date', today())->sum('amount');
    ...
    public function activities(Request $request)
    {
        ...
        $rows = DB::query()
            ->fromSub($saving->unionAll($withdrawal)->unionAll($payment), 'activities')
            ->orderByDesc('date')
            ->limit($limit)
            ->get();
