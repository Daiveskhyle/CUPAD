...
if($method==='GET' && $route==='clients'){
    ...
    elseif($role==='bm' && $user['branch_id']!==''){ $where.=' AND branch_id=?'; $params[]=$user['branch_id']; }
    elseif($role==='am' && $user['area_id']!==''){ $where.=' AND branch_id IN (SELECT id FROM branches WHERE area_id=?)'; $params[]=$user['area_id']; }
    elseif(in_array($role,['zm','dzm','tm'],true) && $user['zone_id']!==''){ $where.=' AND branch_id IN (SELECT id FROM branches WHERE zone_id=? OR area_id IN (SELECT id FROM areas WHERE zone_id=?))'; $params[]=$user['zone_id']; $params[]=$user['zone_id']; }
...
if($method==='GET' && preg_match('#^clients/([^/]+)/portfolio$#',$route,$m)){ requireJwt(); respond(['success'=>true,'data'=>portfolioData($m[1])]); }
if($method==='GET' && preg_match('#^clients/([^/]+)/savings$#',$route,$m)){ requireJwt(); $id=$m[1]; if(!clientExists($id)) respond(['success'=>false,'error'=>'Client not found'],404); $s=db()->prepare('SELECT * FROM savings WHERE client_id=? ORDER BY created_at DESC'); $s->execute([$id]); respond(['success'=>true,'data'=>$s->fetchAll()]); }
if($method==='GET' && preg_match('#^clients/([^/]+)/loans$#',$route,$m)){ requireJwt(); $id=$m[1]; if(!clientExists($id)) respond(['success'=>false,'error'=>'Client not found'],404); $s=db()->prepare('SELECT id,principal,interest_rate,total_payable,remaining_balance,num_installments,loan_term_type,date,due_date,payoff_date,status FROM disbursements WHERE client_id=? ORDER BY date DESC'); $s->execute([$id]); respond(['success'=>true,'data'=>$s->fetchAll()]); }
if($method==='GET' && preg_match('#^clients/([^/]+)/transactions$#',$route,$m)){ requireJwt(); $id=$m[1]; if(!clientExists($id)) respond(['success'=>false,'error'=>'Client not found'],404); $s=db()->prepare("SELECT transaction_id,'savings' source,amount,type,date,balance_after,notes FROM saving_collections WHERE client_id=? UNION ALL SELECT transaction_id,'loan' source,amount_collected amount,type,date,remaining_balance balance_after,notes FROM loan_collections WHERE client_id=? ORDER BY date DESC LIMIT 200"); $s->execute([$id,$id]); respond(['success'=>true,'data'=>$s->fetchAll()]); }
if($method==='GET' && preg_match('#^portfolio/([^/]+)$#',$route,$m)){ requireJwt(); respond(['success'=>true,'data'=>portfolioData($m[1])]); }

/* Write operations and officer activity used by cupad-mobile. */
