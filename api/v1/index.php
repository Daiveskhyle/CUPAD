...
if($method==='GET' && $route==='health'){
    try { db()->query('SELECT 1'); respond(['success'=>true,'service'=>'CUPAD API','version'=>'1.1.1','database'=>'ok']); }
    catch(Throwable $e){ respond(['success'=>false,'database'=>'error'],503); }
}
if($method==='GET' && $route==='test'){
    requireApiKey();
    try { db()->query('SELECT 1'); respond(['success'=>true,'authenticated'=>true,'service'=>'CUPAD API','version'=>'1.1.1','database'=>'ok','time'=>date('c')]); }
...
/* Mobile clients endpoint: JWT is now accepted so the mobile app never needs
   a server API key embedded in the application. Role scoping is applied. */
if($method==='GET' && $route==='clients'){
    $auth=requireJwt();
    $userId=(int)($auth['sub']??0);
    $uStmt=db()->prepare('SELECT id,username,role,branch_id,area_id,zone_id FROM users WHERE id=? AND status="active" LIMIT 1');
    $uStmt->execute([$userId]); $user=$uStmt->fetch();
    ...
    if($role==='co'){
        $where.=' AND officer_username=?'; $params[]=$user['username'];
        ...
        $where.=' AND branch_id IN (SELECT id FROM branches WHERE area_id=?)'; $params[]=$user['area_id'];
        ...
        $params[]=$user['zone_id']; $params[]=$user['zone_id'];
    }
    if($q!==''){
        $where.=' AND (id LIKE ? OR name LIKE ? OR phone LIKE ?)'; $like="%$q%";
...
if($method==='GET' && preg_match('#^clients/([^/]+)/savings$#',$route,$m)){
    requireJwt(); $id=$m[1]; if(!clientExists($id)) respond(['success'=>false,'error'=>'Client not found'],404);
