...
if (isset($_GET['ajax_action'])) {
    ...
    if ($_GET['ajax_action'] === 'get_union_data') {
        ...
        $stmt = $pdo->prepare("SELECT id, name FROM clients WHERE officer_username = ? AND status = 'active' AND `union` = ? ORDER BY name ASC");
        $stmt->execute([$username, $union]);
        ...
        if (count($clients) > 0) {
            ...
            // 2. Bulk Fetch Active Loans (Include loans that got completed ON or AFTER the requested date)
            $stmt_act_l = $pdo->prepare("SELECT * FROM disbursements WHERE (status != 'completed' OR payoff_date >= ?) AND client_id IN ($placeholders) ORDER BY date DESC");
            $params_act = array_merge([$query_date], $client_ids);
            $stmt_act_l->execute($params_act);
            $active_loans =[];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'save_client') {
    ...
    $existing_pic_path = null;

    // Fetch existing image note if any to allow amount updates without re-uploading
    $chk_existing_img = $pdo->prepare("SELECT notes FROM saving_collections WHERE client_id = ? AND DATE(date) = ? AND type = 'cash' LIMIT 1");
    $chk_existing_img->execute([$client_id, $payment_date]);
    if ($ex_row = $chk_existing_img->fetch(PDO::FETCH_ASSOC)) {
        if (!empty($ex_row['notes']) && strpos($ex_row['notes'], 'Image:') !== false) {
            $existing_pic_path = 'uploads/withdrawals/' . trim(str_replace('Image:', '', $ex_row['notes']));
        }
    }

    if ($withdrawal_type === 'cash') {
        if (!isset($_FILES['picture']) || $_FILES['picture']['error'] !== UPLOAD_ERR_OK) {
            if (empty($existing_pic_path) && intval($w_settings['require_image_for_cash']) === 1) {
                echo json_encode(['success' => false, 'message' => 'Proof image is required for cash withdrawals.']); exit;
            } else {
                $uploaded_pic_path = $existing_pic_path; // Retain existing image
            }
        } else {
    ...
    try {
        ...
        // PRE-FETCH GLOBAL SAVINGS ID & BALANCE
        ...
        // PRE-FETCH EXISTING TRANSACTIONS FOR UPDATE
        $chk_l = $pdo->prepare("SELECT transaction_id, amount_collected, disbursement_id FROM loan_collections WHERE client_id = ? AND DATE(date) = ? FOR UPDATE");
        $chk_l->execute([$client_id, $payment_date]);
        $existing_loan_tx = $chk_l->fetch(PDO::FETCH_ASSOC);

        $chk_s = $pdo->prepare("SELECT transaction_id, amount FROM saving_collections WHERE client_id = ? AND DATE(date) = ? AND type = 'deposit' FOR UPDATE");
        $chk_s->execute([$client_id, $payment_date]);
        $existing_sav_tx = $chk_s->fetch(PDO::FETCH_ASSOC);

        $chk_w = $pdo->prepare("SELECT transaction_id, amount, type FROM saving_collections WHERE client_id = ? AND DATE(date) = ? AND type IN ('cash','withdrawal','return') FOR UPDATE");
        $chk_w->execute([$client_id, $payment_date]);
        $existing_wth_tx = $chk_w->fetch(PDO::FETCH_ASSOC);

        // PRE-FETCH ACTIVE LOAN
        $stmt = $pdo->prepare("SELECT * FROM disbursements WHERE client_id = ? AND (status != 'completed' OR payoff_date >= ?) ORDER BY date DESC LIMIT 1");
        $stmt->execute([$client_id, $payment_date]);
        $loan = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$loan && $existing_loan_tx && !empty($existing_loan_tx['disbursement_id'])) {
        ...
        // =======================================================
        // 1. REVERT EXISTING WITHDRAWAL (Do this first to free up loan/saving DB rules)
        // =======================================================
        if ($existing_wth_tx) {
            $old_deduct = abs(floatval($existing_wth_tx['amount']));
            $old_type = $existing_wth_tx['type'];

            $current_balance += $old_deduct;
            $updateSavingsBal($client_id, $savings_id, $current_balance);
            
            if (($old_type === 'withdrawal' || $old_type === 'return') && $loan) {
                $revertDisbursementBal($loan['id'], $old_deduct);
                $loan['remaining_balance'] += $old_deduct; // SYNC MEMORY VARIABLE
            }
        ...
        // Outstanding Balance = 0 Rule
        if ($existing_loan_tx && $existing_loan_tx['disbursement_id']) {
            $old_amt = floatval($existing_loan_tx['amount_collected']);
            $ex_disb_id = $existing_loan_tx['disbursement_id'];
            $new_disb_id = ($pay_amt > 0 && $loan) ? $loan['id'] : null;
            $effective_new_amt = ($ex_disb_id == $new_disb_id) ? $pay_amt : 0;

            if ($effective_new_amt < $old_amt) {
                $stmt_chk_bal = $pdo->prepare("SELECT remaining_balance FROM disbursements WHERE id = ?");
                $stmt_chk_bal->execute([$ex_disb_id]);
                if (floatval($stmt_chk_bal->fetchColumn()) <= 0.01) {
                    $stmt_newer = $pdo->prepare("SELECT COUNT(*) FROM disbursements WHERE client_id = ? AND id != ? AND DATE(date) >= ?");
                    $stmt_newer->execute([$client_id, $ex_disb_id, $payment_date]);
                    if ($stmt_newer->fetchColumn() > 0) {
                        throw new Exception("Cannot reduce/clear this repayment. It paid off a previous loan, and a new loan has already been disbursed.");
                    }
                }
            }
        }
        ...
        if ($pay_amt > 0 && $loan) {
            ...
            if ($existing_loan_tx) {
                ...
                if ($existing_disb_id && $existing_disb_id == $disb_id) {
                    ...
                    $pdo->prepare("UPDATE loan_collections SET amount_collected = ?, remaining_balance = ? WHERE transaction_id = ?")->execute([$pay_amt, $new_rem, $existing_loan_tx['transaction_id']]);
                    $loan['remaining_balance'] = $new_rem; // SYNC MEMORY
                    ...
                    $pdo->prepare("DELETE FROM loan_collections WHERE transaction_id = ?")->execute([$existing_loan_tx['transaction_id']]);
            ...
            if ($disb_id) {
                $revertDisbursementBal($disb_id, floatval($existing_loan_tx['amount_collected']));
                $loan['remaining_balance'] += floatval($existing_loan_tx['amount_collected']); // SYNC MEMORY
            }
            $pdo->prepare("DELETE FROM loan_collections WHERE transaction_id = ?")->execute([$existing_loan_tx['transaction_id']]);
        ...
        if ($saving_amt > 0 || $existing_sav_tx) {
            // COLLATERAL CHECK relies on synchronized memory variable
            ...
            if ($existing_sav_tx) {
                ...
                $current_balance += $diff; // SYNC MEMORY

                if ($saving_amt > 0) {
                    $updateSavingsBal($client_id, $savings_id, $current_balance);
                    $pdo->prepare("UPDATE saving_collections SET amount = ?, balance_after = ? WHERE transaction_id = ?")->execute([$saving_amt, $current_balance, $existing_sav_tx['transaction_id']]);
                    $client_updates[] = "Savings Updated";
                } else {
                    $updateSavingsBal($client_id, $savings_id, $current_balance);
                    $pdo->prepare("DELETE FROM saving_collections WHERE transaction_id = ?")->execute([$existing_sav_tx['transaction_id']]);
                    $client_updates[] = "Savings Removed";
                }
            } elseif ($saving_amt > 0) {
                $current_balance += $saving_amt; // SYNC MEMORY
                $updateSavingsBal($client_id, $savings_id, $current_balance);
                ...
                $pdo->prepare("INSERT INTO saving_collections (transaction_id, client_id, savings_id, amount, type, date, officer, balance_after, created_at) VALUES (?, ?, ?, ?, 'deposit', ?, ?, ?, NOW())")
                    ->execute([$tx_id, $client_id, $savings_id, $saving_amt, $date_fmt, $username, $current_balance]);
        ...
        // 4. WITHDRAWAL APPLY
        // =======================================================
        if ($withdrawal_type !== '' && ($withdrawal_amt > 0 || $withdrawal_type === 'return')) {
            ...
            if ($existing_wth_tx) {
                // Technically it's deleted now, but keeping check logic for safety
            ...
            if ($withdrawal_type === 'return' && $loan) {
                $actual_deduct = min($current_balance, floatval($loan['remaining_balance']));
            }

            if ($actual_deduct <= 0) {
                throw new Exception("Calculated withdrawal amount is zero or insufficient savings.");
            }

            $current_balance -= $actual_deduct; // SYNC MEMORY
            ...
            if (($withdrawal_type === 'withdrawal' || $withdrawal_type === 'return') && $loan) {
                $loan['remaining_balance'] = max(0, floatval($loan['remaining_balance']) - $actual_deduct); // SYNC MEMORY
                $updateDisbursementBal($loan['id'], $loan['remaining_balance'], $payment_date);
            }

            $tx_prefix = ($withdrawal_type==='return' ? 'RTN-' : ($withdrawal_type==='cash' ? 'CSH-' : 'WTH-'));
            $tx_id = $tx_prefix . strtoupper(generate_uuid());
            
            $note = ($uploaded_pic_path ? "Image: " . basename($uploaded_pic_path) : null);
            
            $pdo->prepare("INSERT INTO saving_collections (transaction_id, client_id, savings_id, amount, type, date, officer, balance_after, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())")
                ->execute([$tx_id, $client_id, $savings_id, -$actual_deduct, $withdrawal_type, $date_fmt, $username, $current_balance, $note]);
        ...
        $pdo->rollback();
        // Clean up orphan file upload if DB transaction fails, ONLY if it's a freshly uploaded file
        if ($uploaded_pic_path && $uploaded_pic_path !== $existing_pic_path && file_exists('../' . $uploaded_pic_path)) {
...
$stmt = $pdo->prepare("SELECT id, name, `union` FROM clients WHERE officer_username = ? AND status='active' ORDER BY name ASC");
$stmt->execute([$username]);

while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $u = $row['union'] ?? 'Unassigned';
    if (!isset($clients_by_union[$u])) { $clients_by_union[$u] =[]; $assigned_unions[] = $u; }
...
<body>
            ...
            <div class="controls-layout">
                <div class="flex-1">
                    <div class="controls-header">
                        <label class="label-heading">Select Union</label>
    ...
    <nav class="mobile-bottom-nav">
        ...
        <a href="saving_withdrawal.php" class="nav-item">
    ...
    <script>
        ...
        function init() {
            ...
            if(unions.length === 0) {
                ...
                dom.tableBody.innerHTML = '<tr><td colspan="4" class="empty-state">No assigned unions</td></tr>';
            } else {
                unions.forEach((u, i) => {
        ...
        async function selectUnion(union, el) {
            ...
            const clientCount = clientsByUnion[union]?.length || 0;
            let skeletonHtml = '';
            ...
            try {
                const res = await fetch(`?ajax_action=get_union_data&union=${encodeURIComponent(union)}&date=${encodeURIComponent(dateVal)}`);
        ...
        function renderTable(clientData) {
            ...
            clientData.forEach(c => {
                ...
                if (c.loan) {
                    ...
                    if(existing.wth_type === 'withdrawal' || existing.wth_type === 'return') {
                        loanRem += existing.wth_amt;
                    }
                ...
                <tr class="client-row ${visitedClass}" data-name="${safeName.toLowerCase()}" id="row_${c.id}">
                    ...
                    <td class="col-wth-cell">
                        <div class="flex gap-1 mb-1">
                            ...
                            <label id="pic_lbl_${c.id}" for="pic_${c.id}" class="pic-label ${existing.wth_type==='cash'?'flex active':'hidden'} ${blockWth?'disabled':''}" title="Attach Proof">
        ...
        function handleWthSelect(sel, cid) {
            ...
            if(val === 'return') { 
                ...
                const currSav = sInp ? (parseFloat(sInp.value) || 0) : 0;
                
                const outDisp = document.getElementById(`disp_out_${cid}`);
                const baseRem = outDisp ? parseFloat(outDisp.dataset.baseRem) || 0 : 0;
                
                const totalSav = baseSav + currSav;
                const maxReturn = Math.min(totalSav, baseRem);
                
                wAmt.value = maxReturn > 0 ? Math.round(maxReturn) : '';
                wAmt.readOnly = true;
            ...
            // Defer autosave for Cash until an image is provided
            if (val !== 'cash' || parseInt(wSettings.require_image_for_cash) !== 1) {
                triggerAutoSave(cid);
            } else {
                updateRowStatus(cid, 'waiting_image');
            }
        }
        ...
        function triggerAutoSave(cid) {
            ...
            // Check if it's a cash withdrawal that requires an image, but no image is attached yet
            if (wSel && wSel.value === 'cash' && parseInt(wSettings.require_image_for_cash) === 1) {
                const wPicLbl = document.getElementById(`pic_lbl_${cid}`);
                const hasExistingImg = wPicLbl && wPicLbl.classList.contains('active');
                
                if (!hasExistingImg && (!picInput || picInput.files.length === 0)) {
                    updateRowStatus(cid, 'waiting_image');
                    return; // Abort auto-save until the image is provided
                }
            }
        ...
        function updateRowStatus(cid, state) {
            ...
            if (state === 'pending') {
                ...
                statusEl.innerHTML = '<span class="text-warning" style="display:flex;align-items:center;gap:0.25rem;font-size:0.65rem;"><i class="fas fa-camera"></i>Photo Rq.</span>';
                rowEl.classList.add('row-edited');
            ...
            calcTimeout = setTimeout(() => {
                ...
                rows.forEach(tr => {
                    ...
                    if(wSel && wSel.value) {
                        ...
                        } else if(wType === 'cash' || wType === 'withdrawal') {
                            wVal = (parseFloat(wAmt.value)||0);
                        }
                        tW += wVal;
                    }
