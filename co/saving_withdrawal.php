...
// co/saving_withdrawal.php - Fixed: No Double Record in Loan History
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['client_id'])) {
    ...
    if (in_array($type, $blocked_types)) {
        ...
        try {
            ...
            // 1. Get Client & Current Balance (Locking Read)
            $stmt = $pdo->prepare("
                SELECT c.name, c.union, sb.balance 
                FROM clients c 
            ...
            elseif ($type === 'return' || $type === 'withdrawal') {
                ...
                // Update Loan Record (Deduct from active loan ONLY)
                if ($active_loan) {
                    ...
                    // UPDATE THE BALANCE ONLY - NO INSERT INTO LOAN_COLLECTIONS
                    ...
                    // NOTE: Removed insertion into loan_collections to prevent double record.
                    // The loan balance decreases, but no "Repayment" record is generated.
                }
            }

            // 6. Insert Saving Transaction (Negative Amount for withdrawal)
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            // Note: Saving amount is stored as negative for withdrawals
...
if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
    ...
    $where = "WHERE sc.amount < 0 AND sc.officer = ?";
    $params = [$co_username];
    ...
    $sql = "SELECT sc.transaction_id, sc.date, ABS(sc.amount) as amount, sc.type, c.name as client 
...
// --- DATA PREPARATION FOR VIEW (Using SQL) ---

// 1. Get Clients for this CO
$clients = [];
$clients_by_union = [];
$client_data_js = [];
$total_savings = 0;
...
$stmt = $pdo->prepare("
    SELECT c.id, c.name, c.union, sb.balance as savings_balance 
    FROM clients c
    LEFT JOIN saving_balances sb ON c.id = sb.client_id
...
foreach ($raw_clients as $c) {
    ...
    // Calculate Next Loan Limit (Based on previous max)
    $max_prev_stmt = $pdo->prepare("SELECT MAX(principal) FROM disbursements WHERE client_id = ?");
    $max_prev_stmt->execute([$cid]);
    $max_prev = floatval($max_prev_stmt->fetchColumn() ?: 0);
    ...
    // --- NEW: Calculate Installments Paid for JS display ---
    ...
    $client_data_js[] = [
        ...
        'installments_paid' => $installments_paid, // Passed to JS
        'active_loan_principal' => $active_principal,
        'next_loan_limit' => $next_limit
    ];
}

// 3. Stats for Dashboard (Current Month)
$current_month = date('Y-m');
$stmt = $pdo->prepare("
    SELECT COUNT(*) as cnt, ABS(SUM(amount)) as total 
    FROM saving_collections 
    WHERE officer = ? AND amount < 0 AND DATE_FORMAT(date, '%Y-%m') = ?
");
...
// 4. Initial History Load (Today's Data for Local JS Check)
$today = date('Y-m-d');
$stmt = $pdo->prepare("
    SELECT client_id, date, type 
    FROM saving_collections 
    WHERE officer = ? AND DATE(date) = ? AND amount < 0
");
$stmt->execute([$co_username, $today]);
$withdrawal_history_data = $stmt->fetchAll();

// User Profile
$profile_pic = 'default_avatar.png';
$u_stmt = $pdo->prepare("SELECT profile_pic FROM users WHERE username = ?");
$u_stmt->execute([$co_username]);
$u_res = $u_stmt->fetch();
...
<head>
    ...
    <style>
        ...
        .modal-overlay.active { 
            opacity: 1; 
            display: flex; /* FIXED: Make sure it becomes flex */
...
<body>
        ...
        <nav class="navbar">
            ...
            <div class="flex items-center gap-3">
                <?php if ($_SESSION['user_role'] === 'admin'): ?>
                    <a href="../admin/dashboard.php" class="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-full transition" title="Admin Dashboard">
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Process payouts and loan payments</p>
        </div>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-card card-gradient-green">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        <div class="text-sm font-semibold uppercase opacity-90">Active Savers</div>
                    ...
                    <form id="withdrawalForm" method="POST" enctype="multipart/form-data" class="p-5 space-y-5">
                        ...
                        <div id="client-info-card" class="hidden rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-800 shadow-sm mb-4 transition-all duration-300">
                            ...
                            <div class="p-4 grid grid-cols-2 gap-y-4 gap-x-6">
                                ...
                                <div id="eligibility-container" class="col-span-2 mt-2 hidden">
                                    <!-- Populated by JS -->
                                </div>
                        ...
                        <div>
                            ...
                            <div class="input-group">
                                <input type="datetime-local" id="date" name="date" value="<?php echo date('Y-m-d\TH:i'); ?>" <?php echo ($w_settings['withdrawal_date_readonly']??0) ? 'readonly' : ''; ?> class="input-field text-sm p-0">
                            </div>
                        </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 min-h-[500px] flex flex-col">
                    <div class="p-5 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                        ...
                        <div class="flex gap-2">
                            <select id="history-union-filter" class="text-xs bg-gray-100 dark:bg-slate-700 border-none rounded-lg px-2 py-1 outline-none">
    ...
    <script>
        ...
        const assignedUnions = Object.keys(clientsByUnion);
        ...
        // Local history for daily check (From daily sql fetch)
        const withdrawalHistory = <?php echo json_encode($withdrawal_history_data); ?>;

        let historyState = { page: 1, union: 'all', type: 'all', loading: false, hasMore: true };
        ...
        function initUnions() {
            ...
            if(!unions.length) els.unionTags.innerHTML = '<span class="text-xs italic text-gray-400">No unions assigned</span>';
        ...
        function initWithdrawalTypes() {
            ...
            // If current selected type is blocked, select first available
        ...
        function selectUnion(union, btnEl) {
            ...
            btnEl.className = 'px-3 py-1 rounded-full text-xs font-bold border transition bg-red-600 text-white border-red-600';
            els.clientSelect.innerHTML = '<option value="">Select Client...</option>';
            if(clientsByUnion[union]) clientsByUnion[union].forEach(c => els.clientSelect.innerHTML += `<option value="${c.id}">${c.name}</option>`);
            updateFormUI();
        }
        ...
        function updateFormUI() {
            ...
            const client = clientData.find(c => c.id == cid); // == for type coercion
            
            // Additional elements for new detailed card
            ...
            // Update buffer percentage display based on withdrawal type
            ...
            if(client) {
                ...
                // 2. Loan Plan & Installment Count Logic
                if (client.loan_balance > 0) {
                    ...
                    // --- SHOW 10th INSTALLMENT ELIGIBILITY NOTIFICATION ---
                ...
                // 3. Savings Health Check UI (Granular % Rule)
        ...
        // --- SUBMIT LOGIC with Improved Checks ---
        els.preSubmitBtn.onclick = (e) => {
            ...
            // Check if withdrawal type is blocked
            if (blockedTypes.includes(type)) {
                showToast('This withdrawal type is currently blocked', 'error');
                return;
            }
            ...
            // 2. DAILY LIMIT CHECK (Client-Side Pre-check using local history)
            const selDateStr = selectedDate.split('T')[0];
            const maxDaily = withdrawalSettings.max_withdrawals_per_day || 1;
            let dailyCount = 0;
            
            withdrawalHistory.forEach(tx => {
                let txId = tx.client_id;
                if(txId == client.id && tx.date.startsWith(selDateStr)) {
                    dailyCount++;
                }
            });
            
            if(dailyCount >= maxDaily) {
                showToast(`Daily limit reached for this client (${maxDaily} max)`, 'warning');
                return;
            }
            ...
            if (type === 'return') {
                ...
                els.modal.classList.add('active'); // Show confirmation modal
                return;
            } 
        ...
        // History Load
        if(assignedUnions.length) assignedUnions.forEach(u => els.historyUnionFilter.innerHTML += `<option value="${u}">${u}</option>`);
        els.historyUnionFilter.onchange = (e) => { historyState.union = e.target.value; historyState.page = 1; loadHistory(true); };
        ...
        async function loadHistory(reset) {
            ...
            try {
                const res = await fetch(`?ajax=1&page=${historyState.page}&union=${historyState.union}`);
                        ...
                        <div class="activity-card">
                            ...
                            <div class="ac-content">
                                ...
                                <div class="ac-bottom">
                                    ...
                                    <div class="activity-date"><i class="far fa-clock"></i> ${item.date.substring(5,16)}</div>
                                </div>
                            </div>
                        </div>`;
        ...
        function animateNumbers() {
            document.querySelectorAll('.count-up').forEach(el => {
                ...
                const anim = (ts) => {
                    ...
                    el.innerText = Math.round(p*t).toLocaleString();
